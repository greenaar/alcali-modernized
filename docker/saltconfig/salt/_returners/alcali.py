...
def _get_options(ret=None):
    ...
    _options = salt.returners.get_returner_options(
        "mysql", ret, attrs, __salt__=__salt__, __opts__=__opts__, defaults=defaults
    )
    # post processing
...
def returner(ret):
    ...
    try:
        ...
        log.critical(exc)
        log.critical(
            "Could not store return with Alcali returner. MySQL server unavailable."
        )
...
def event_return(events):
    ...
    with _get_serv(events, commit=True) as cur:
        for event in events:
            ...
            # TODO: check overhead.
            if tag in ["salt/auth", "salt/key"]:
                cur.execute("TRUNCATE TABLE `salt_keys`")
                key = salt.key.get_key(__opts__)
                keys = key.list_keys()
                for status, keys_list in keys.items():
                    if keys_list:
                        for minion_id in keys_list:
                            finger_ret = key.finger(minion_id, hash_type="sha256")
                            finger = finger_ret[status][minion_id]
                            sql = """INSERT INTO `salt_keys` (`minion_id`, `pub`, `status`)
                                     VALUES (%s, %s, %s)"""
                            cur.execute(sql, (minion_id, finger, keys_status[status]))
